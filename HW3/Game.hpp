...
public:
	...
	static vector<bool> vstring_to_vbool(const vector<string>& arg);

    void calculate_next_generation_matrix(int starting_line, int ending_line);
    int calculate_live_neighbours(int current_line, int current_cell_num);
...
protected: // All members here are protected, instead of private for testing purposes
    ...
    void global_lock_initialize();


	string file_name;
	uint input_width;
	uint input_height;
	bool_mat* curr_matrix;
	bool_mat* next_matrix;
	PCQueue<Job*>* pcq;
    pthread_mutex_t global_lock;
    pthread_mutexattr_t attribute;
    pthread_cond_t global_condition;

    bool finished;
    uint finished_thread_counter;
