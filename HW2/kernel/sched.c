...
struct runqueue {
	...
	task_t *curr, *idle;
	prio_array_t *active, *expired, *short_pr, arrays[3]; /* HW2 added */
...
#define rt_task(p)		(((p)->policy != SCHED_SHORT) && ((p)->prio < MAX_RT_PRIO)) /* HW2 added */
...
repeat_lock_task:
	...
	if (!p->array) {
		 ...
		 */
		/* HW2 added */
		//check if we should resched task - 
		//if policy disabled - do normally, otherwise, policy is enabled and then if its a changeable 
		//process, run only if its the lowest pid, otherwise it's not changeble, then run if prio is lower
		if((p->policy == SCHED_SHORT && rq->curr->policy == SCHED_OTHER) ||
		  (p->policy == SCHED_SHORT && rq->curr->policy == SCHED_SHORT && (p->short_prio < rq->curr->short_prio)) ||
		  (rt_task(p) && rq->curr->policy == SCHED_SHORT)){
...
{
	__cli();
	if (p->first_time_slice && p->policy != SCHED_SHORT) { // HW2 added - not adding to father the left timeslice if son was short
...
void scheduler_tick(int user_tick, int system)
{
	...
	if (p->array != rq->active && p->array != rq->short_pr) {
		set_tsk_need_resched(p);
		return;
	}
	spin_lock(&rq->lock);
...
short_proc: /* HW2 added */
	if (!--p->time_left) {
		dequeue_task(p, rq->short_pr);
		p->policy = SCHED_OTHER;
		p->static_prio = ((p->static_prio + 7) < (MAX_PRIO - 1))? (p->static_prio + 7):(MAX_PRIO - 1);
...
pick_next_task:
	...
	idx = sched_find_first_bit(array->bitmap);
	if (idx >= MAX_RT_PRIO) {
		idx_short = sched_find_first_bit(rq->short_pr->bitmap);
		if(idx_short < MAX_PRIO){
			queue = rq->short_pr->queue + idx_short;
...
static int setscheduler(pid_t pid, int policy, struct sched_param *param)
{
	...
	array = p->array;
	/* HW2 added */ 
	if ((policy == SCHED_SHORT &&  ((lp.requested_time < 1) || (lp.requested_time > 3000) || (lp.sched_short_prio < 0) || (lp.sched_short_prio > 139)))){
		retval = -EINVAL;
		goto out_unlock;
	}

		if((p->policy == SCHED_SHORT) || ((p->policy != SCHED_OTHER) && (policy == SCHED_SHORT))){
		retval = -EPERM;
		goto out_unlock;
	}
	...
	if (policy < 0)
		...
		if (policy != SCHED_FIFO && policy != SCHED_RR &&
				policy != SCHED_OTHER && policy != SCHED_SHORT) // HW2 added
			goto out_unlock;
	}
	...
	if (policy != SCHED_OTHER && p->policy != SCHED_SHORT)
		p->prio = MAX_USER_RT_PRIO-1 - p->rt_priority;


/* HW2 added */  
	else if(policy == SCHED_SHORT){ // need to improve !!
		p->short_prio = lp.sched_short_prio;
		p->requested_time = lp.requested_time;
		p->time_left = (((lp.requested_time * HZ)/1000) == 0)?1:(lp.requested_time * HZ)/1000;
	}
...
asmlinkage long sys_sched_yield(void)
{
	...
	// end added the original code in else - if not short do as usuall 
	list_del(&current->run_list);
...
void __init sched_init(void)
{
	...
	for (i = 0; i < NR_CPUS; i++) {
		...
		rq->short_pr = rq->arrays + 2; /* HW2 added */
		spin_lock_init(&rq->lock);
		INIT_LIST_HEAD(&rq->migration_queue);

		for (j = 0; j < 3; j++) { /* HW2 added */
			array = rq->arrays + j;
...
/* HW2 added */
 ...
 int sys_is_short(pid_t pid) {
    //printk("you called 243, with pid %d\n", pid);
	...
	struct task_struct* t = find_task_by_pid(pid);//find the process with our pid
	if(t == NULL){
		return -ESRCH;
	}
    errno = temp;
...
int sys_short_remaining_time(pid_t pid) {
    ...
    task_t* process = find_task_by_pid(pid);
    if (process == NULL) {
        return -ESRCH;
    }
    if (process->policy != SCHED_SHORT){
        return -EINVAL;
    }
    errno = temp;
		return (((process->time_left * 1000)/HZ));
}
...
int sys_short_place_in_queue(pid_t pid) {
	...
	runqueue_t *rq;
    rq = this_rq();
    list_t* iterator , *curr_queue;
	task_t* current_task;
	prio_array_t* array;
	array = rq->short_pr;
	curr_queue = array->queue;
	int counter=0,curr_prio;
	for(curr_prio = 0; curr_prio <= process->short_prio; curr_prio++){
			curr_queue = array->queue + curr_prio;
			list_for_each(iterator, curr_queue){
				current_task = list_entry(iterator, task_t, run_list);
				if(current_task->pid == process->pid){
					return counter;
				}else{
					counter++;
				}
